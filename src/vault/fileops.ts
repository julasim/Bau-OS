...
function isBinary(filepath: string): boolean {
  try {
    ...
    if (stat.size > 1_048_576) return true; // > 1MB
...
export function editFile(
  ...
  // Preview: erste Ersetzung im Kontext zeigen
...
export function grepFiles(
  ...
  try {
    ...
    throw new Error(`Ungueltiger Regex: ${err}`);
  }

  // Optional: Datei-Filter
  let fileRegex: RegExp | null = null;
  if (options?.fileGlob) {
    fileRegex = globToRegex(options.fileGlob);
  }
