...
import {
  COMMAND_TIMEOUT_SEC, COMMAND_TIMEOUT_MAX_SEC,
  COMMAND_BUFFER_SIZE, TOOL_OUTPUT_MAX_CHARS,
  VM_TIMEOUT_MS, CODE_OUTPUT_MAX_CHARS,
} from "../../config.js";
import type { HandlerMap } from "./types.js";

export const systemSchemas: OpenAI.Chat.ChatCompletionTool[] = [
  { type: "function", function: { name: "befehl_ausfuehren", description: "Fuehrt einen Shell-Befehl auf dem Server aus. Fuer: Systeminfo (df -h, uptime, free -h, top -bn1), Dateien (ls, cat, wc, head, tail, grep, find), Services (systemctl status), Netzwerk (curl, ping, dig), Pakete (apt list), Prozesse (ps aux), Logs (journalctl -u bau-os -n 50). Befehle koennen mit | verkettet werden. Destruktive Befehle (rm -rf, shutdown, reboot) sind blockiert.", parameters: { type: "object", properties: { befehl: { type: "string", description: "Shell-Befehl (z.B. 'df -h', 'cat /etc/hostname', 'ps aux | grep node')" }, verzeichnis: { type: "string", description: "Optionales Arbeitsverzeichnis (Standard: /opt/bau-os)" }, timeout: { type: "number", description: "Timeout in Sekunden (Standard: 15, max: 60)" } }, required: ["befehl"] } } },
  { type: "function", function: { name: "code_ausfuehren", description: "Fuehrt JavaScript-Code direkt auf dem Server aus. Fuer: Berechnungen (Flaechen, Kosten, Prozent), Daten transformieren (JSON parsen, CSV verarbeiten, Datumsberechnungen), Text verarbeiten (Regex, Split, Format). Der Code laeuft in Node.js — alle eingebauten Module verfuegbar (fs, path, crypto etc.). Letzter Ausdruck wird als Ergebnis zurueckgegeben.", parameters: { type: "object", properties: { code: { type: "string", description: "JavaScript-Code (z.B. 'Math.round(125.5 * 0.2 * 100) / 100' oder mehrzeiliger Code)" } }, required: ["code"] } } },
];
...
export const systemHandlers: HandlerMap = {
  befehl_ausfuehren: async (args) => {
    ...
    return new Promise<string>((resolve) => {
      exec(cmd, { timeout: timeoutMs, maxBuffer: COMMAND_BUFFER_SIZE, cwd, env: { ...process.env, LANG: "de_AT.UTF-8" } }, (error, stdout, stderr) => {
        if (error) {
        ...
        resolve(output.length > TOOL_OUTPUT_MAX_CHARS ? output.slice(0, TOOL_OUTPUT_MAX_CHARS) + `\n\n[... gekuerzt, ${output.length - TOOL_OUTPUT_MAX_CHARS} Zeichen entfernt]` : output || "(kein Output)");
      });
    });
  },
  ...
  code_ausfuehren: async (args) => {
    ...
    try {
      ...
      return full.length > CODE_OUTPUT_MAX_CHARS ? full.slice(0, CODE_OUTPUT_MAX_CHARS) + "\n[... gekuerzt]" : full || "(kein Ergebnis)";
    } catch (err) {
