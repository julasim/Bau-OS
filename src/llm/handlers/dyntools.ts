...
export const dyntoolSchemas: OpenAI.Chat.ChatCompletionTool[] = [
  {
    ...
    function: {
      name: "tool_erstellen",
      description:
        "Erstellt ein neues wiederverwendbares Tool als Script. Das Tool wird sofort verfuegbar — kein Neustart noetig. Schreibe den Code so, dass er `args.paramName` nutzt und mit `return 'ergebnis'` das Resultat zurueckgibt. Fuer Templates: nutze `files()` um Zusatzdateien im Tool-Ordner zu lesen.",
      parameters: {
        type: "object",
        properties: {
          ...
          parameter: {
            type: "string",
            description: 'Parameter als JSON: {"flaeche": {"type": "number", "description": "m²"}, ...}',
          },
          pflichtfelder: { type: "string", description: "Komma-separierte Pflichtfelder (z.B. 'flaeche,typ')" },
          code: {
            type: "string",
            description:
              "JavaScript-Code des Tools. Zugriff auf args.*, files(), Math, Date, JSON, fetch. Ergebnis via return.",
          },
          typ: { type: "string", description: "Script-Typ: 'js' (Standard) oder 'sh' (Shell)" },
          zusatzdateien: {
            type: "string",
            description: 'Optionale Zusatzdateien als JSON: {"vorlage.md": "# Template\\n..."}',
          },
        },
  ...
  },
  {
    ...
    function: {
      name: "tool_loeschen",
      description:
        "Loescht ein dynamisches Tool dauerhaft (gesamter Ordner). Nicht rueckgaengig machbar. Nutze tools_auflisten um den Ordnernamen zu finden.",
      parameters: {
        type: "object",
        properties: { ordner: { type: "string", description: "Ordnername des Tools (z.B. 'kalkulation')" } },
        required: ["ordner"],
      },
    },
  },
...
export const dyntoolHandlers: HandlerMap = {
  tool_erstellen: async (args) => {
    ...
    if (args.parameter) {
      try {
        params = JSON.parse(String(args.parameter));
      } catch {
        return "Fehler: parameter ist kein gueltiges JSON.";
      }
    }
    const required = args.pflichtfelder
      ? String(args.pflichtfelder)
          .split(",")
          .map((s) => s.trim())
      : [];
    ...
    if (args.zusatzdateien) {
      try {
        extraFiles = JSON.parse(String(args.zusatzdateien));
      } catch {
        return "Fehler: zusatzdateien ist kein gueltiges JSON.";
      }
    }
