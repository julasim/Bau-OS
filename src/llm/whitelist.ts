...
export function buildToolWhitelist(): string {
  ...
  return [
    ...
    "6. **Such-Regel** — wenn der Nutzer nach INHALTEN fragt (Notizen,",
    "   Dateien, Angebote, Protokolle, Kontakte, Projektdaten, Termine",
    "   im Text, technische Details, Zahlen aus Dokumenten): nutze",
    "   ZUERST `semantisch_suchen`. NICHT `vault_suchen` — das findet",
    "   nur System-Dateien (Agent-Config). Wenn die Frage sich auf",
    "   vergangene Gespraeche bezieht (\u201Eder Termin den ich angelegt",
    "   habe\u201C), nutze `chat_suchen`. Kein Mehrfach-Versuch mit",
    "   verschiedenen Such-Tools \u2014 das eine richtige reicht.",
    "",
    sections.join("\n\n"),
  ].join("\n");
}
