...
// Wir testen die Funktionen direkt — muessen aber VAULT_PATH setzen bevor der Import passiert
...
describe("atomicWriteSync", () => {
  it("schreibt Datei atomar (kein .tmp danach)", async () => {
    // Dynamischer Import damit VAULT_PATH gesetzt ist
    const { atomicWriteSync } = await import("../src/vault/helpers.js");
...
describe("safePath", () => {
  it("erlaubt relative Pfade innerhalb des Vaults", async () => {
    const { safePath } = await import("../src/vault/helpers.js");
    // safePath nutzt vaultPath aus dem Modul, das beim Import gesetzt wird
    // Da VAULT_PATH = tmpDir, testen wir manuell
...
describe("ensureDir", () => {
  it("erstellt verschachtelte Ordner", async () => {
    const { ensureDir } = await import("../src/vault/helpers.js");
